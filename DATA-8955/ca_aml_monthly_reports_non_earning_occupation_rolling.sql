...
, rolling_six_deposits AS NOT MATERIALIZED (
    SELECT DISTINCT a.a_patron_id AS patron_id

        , (SUM(credit_cents)/100.0) - (SUM(debit_cents)/100.0) AS r6_deposit_amount
        , COUNT(DISTINCT patron_transaction_id) AS r6_deposits
    FROM ca_edgebook_snapshot.max_accounting_tables_all a
    JOIN ca_aml_monthly_reports.max_identity_users as u
        ON u.patron_id = a.a_patron_id
        AND is_tester = false
    WHERE (
        (
            p_type='deposit'
        ...
        t_changed_at >= reports_start_time_utc_ca_aml_monthly() - INTERVAL '6 months'
        AND t_changed_at < reports_end_time_utc_ca_aml_monthly()
    )
    GROUP BY 1
...
, rolling_twelve_deposits AS NOT MATERIALIZED (
    SELECT DISTINCT a.a_patron_id AS patron_id

        , (SUM(credit_cents)/100.0) - (SUM(debit_cents)/100.0) AS r12_deposit_amount
        , COUNT(DISTINCT patron_transaction_id) AS r12_deposits
    FROM ca_edgebook_snapshot.max_accounting_tables_all a
    JOIN ca_aml_monthly_reports.max_identity_users as u
        ON u.patron_id = a.a_patron_id
        AND is_tester = false
    WHERE (
        ...
        t_changed_at >= reports_start_time_utc_ca_aml_monthly() - INTERVAL '12 months'
        AND t_changed_at < reports_end_time_utc_ca_aml_monthly()
    )
    GROUP BY 1
    HAVING (SUM(credit_cents)/100.0) - (SUM(debit_cents)/100.0) >= 100000
)
...
SELECT
    ...
    , d.deposit_amount
    , rsd.r6_deposits
    , rsd.r6_deposit_amount
    , rtd.r12_deposits
    , rtd.r12_deposit_amount
...
LEFT JOIN ggr 
  USING(patron_id)
LEFT JOIN rolling_six_deposits rsd
    ON rsd.patron_id = d.patron_id
LEFT JOIN rolling_twelve_deposits rtd
    ON rtd.patron_id = d.patron_id
WHERE (
    rsd.r6_deposit_amount >= 50000
    OR rtd.r12_deposit_amount >= 100000
)
